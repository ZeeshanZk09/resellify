...
export async function addProduct(input: AddProductInput) {
  try {
    ...
    console.log('[addProduct] Called with input:', JSON.stringify(inputWithoutImages, null, 2));
    console.log(
      '[addProduct] Images:',
      images ? (Array.isArray(images) ? `${images.length} files` : '1 file') : 'No images'
    );
    ...
    if (input.images) {
      ...
      const imageFiles = Array.isArray(input.images) ? input.images : [input.images];

      // Filter out any non-File objects (in case ImageFile objects were passed)
      const validFiles = imageFiles.filter((img): img is File => img instanceof File);
    ...
    if (input.specifications && input.specifications.length > 0) {
      ...
      const specResult = await addProductSpecsFromForm(productId, input.specifications);
    ...
    if (input.variants && input.variants.length > 0) {
      ...
      const variantResult = await addProductVariantsFromForm(productId, input.variants);
    ...
    if (updatedProduct && updatedProduct.images && updatedProduct.images.length > 0) {
      ...
      const metadata = generateProductMetadata(
        {
          ...updatedProduct,
          shortDescription: updatedProduct.shortDescription ?? updatedProduct.description,
      ...
      await prisma.product.update({
        ...
        data: {
          ...
          structuredData: structuredData as NullableJsonNullValueInput | InputJsonValue,
        },
      });
    }
...
export async function addProductSpecs(
  ...
  try {
    ...
    for (const spec of specs) {
      if (!spec.groupTitle || !spec.keys.length || !spec.values.length) continue;
      ...
      if (!specGroup) {
        ...
        const mergedKeys = Array.from(new Set([...specGroup.keys, ...spec.keys]));
...
export async function addProductSpecsFromForm(
  ...
  try {
    ...
    const manualSpecs = specs.filter(
      (spec) => spec.specGroupTitle && spec.specGroupKeys && spec.specGroupId.startsWith('temp_')
    );
    ...
    for (const spec of manualSpecs) {
      ...
      if (!specGroup) {
        ...
        // Merge keys if new ones are provided
        const mergedKeys = Array.from(new Set([...specGroup.keys, ...spec.specGroupKeys]));
        if (mergedKeys.length > specGroup.keys.length) {
          specGroup = await prisma.specGroup.update({
            where: { id: specGroup.id },
            data: { keys: mergedKeys },
    ...
    for (const spec of existingSpecs) {
      const specGroup = existingSpecGroups.find((sg) => sg.id === spec.specGroupId);
      if (!specGroup) {
        console.warn(`[SPECS] SpecGroup ${spec.specGroupId} not found, skipping`);
        continue;
      }
    ...
    for (const variant of variants) {
      ...
      if (variant.options && variant.options.length > 0) {
        for (const optionLink of variant.options) {
          ...
          if (!option) {
            console.warn(`[VARIANTS] Option ${optionLink.optionId} not found, skipping`);
            continue;
          }
...
export async function updateProduct(input: UpdateProductInput) {
  try {
    ...
    if (input.status !== undefined) {
      ...
      updateData.publishedById = input.status === 'PUBLISHED' ? session.user.id : null;
    }
