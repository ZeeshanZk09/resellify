  ...
  return (
    <div className="flex">
      ...
      {images && showZoom && (
        <div
          className={
            "fixed inset-0 z-[19] flex justify-between items-center flex-col pt-5 pb-10"
          }
        >
          <div
            className={
              "absolute inset-0 backdrop-blur-[5px] bg-[rgba(0,0,0,0.6)]"
            }
            onClick={() => setShowZoom(false)}
          />
          <div
            className={
              "flex w-[90%] h-[85%] bg-white relative overflow-hidden rounded-xl"
            }
          >
            <button
              onClick={() => setShowZoom(false)}
            ...
            {images.map((image, index) => (
              <Image
                ...
                className={cn(
                  ...
                  index === selectedIndex
                    ? "cursor-pointer border-gray-600 hover:border-gary-600"
                    : "opacity-60",
                )}
                onClick={() => setSelectedIndex(index)}
              />
