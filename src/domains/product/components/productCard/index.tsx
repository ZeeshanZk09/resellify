...
export default function ProductCard({
  ...
  const discount = dealPrice && basePrice ? Math.round(100 - (dealPrice / +basePrice) * 100) : 0;
  ...
  return (
    ...
    >
      ...
      >
        <Heart className={`h-4 w-4 text-green-500 ${isFav ? 'fill-green-500' : ''}`} />
      </button>
      ...
      <div className='flex flex-col grow p-4'>
        ...
        {showCategory && category && (
          <span className='text-xs text-gray-500 mb-1 truncate'>{category}</span>
        )}

        {/* Product Name */}
        <h3 className='text-sm font-semibold text-foreground mb-2 line-clamp-2 h-10'>{name}</h3>
            ...
            <div className='flex items-center gap-2'>
              ...
              <button
                ...
                onClick={async (e) => {
                  ...
                  getCartItems().then(({ cartItems }) => {
                    setAddedToCart(cartItems.find((ci) => ci.productId === id) ? true : false);
                  });
                }}
              >
