...
const TodayDealCard = ({
  ...
  useEffect(() => {
    ...
    if (!dealEndTime) {
      ...
      endTimestamp = isNaN(d.getTime()) ? Date.now() + DEFAULT_DEAL_DURATION_MS : d.getTime();
    }
  ...
  return (
    ...
    >
      ...
      <Link href={url} className='block mt-4'>
        <h3 id='deal-title' className='ml-1 text-foreground/95 text-lg font-semibold leading-tight'>
          {productName}
        </h3>
      </Link>
