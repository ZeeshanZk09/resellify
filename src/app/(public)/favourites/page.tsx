...
const initialDataPromise = Promise.all([getFavProducts(), getCartItems()]);

export default function Favourites() {
  return (
    <Suspense fallback={<FavSkeleton />}>
      <FavContent dataPromise={initialDataPromise} />
    </Suspense>
  );
}

function FavSkeleton() {
  const skeletonIds = ['fs1', 'fs2', 'fs3', 'fs4', 'fs5', 'fs6', 'fs7', 'fs8'];
  return (
    <div className='p-6 max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6'>
      {skeletonIds.map((id) => (
        <div key={id} className='max-w-sm border rounded-lg p-4 animate-pulse'>
...
function FavContent({ dataPromise }: { dataPromise: Promise<[any, any]> }) {
  const [favsResult, cartResult] = use(dataPromise);
  const [cartProductIds, setCartProductIds] = useState<Set<string>>(() => {
    const ids = cartResult?.cartItems?.map((item: any) => item.productId) || [];
    return new Set(ids);
  });
  const [favs] = useState<GetFavProducts>(favsResult?.favs || []);
  const [addToCartLoading, setAddToCartLoading] = useState<Set<string>>(new Set());

  const handleAddToCart = async (productId: string, price: number, quantity: number) => {
              ...
              >
                ...
                {isAdding ? (
                  ...
                  <Button
                    ...
                    onClick={() =>
                      handleAddToCart(product.product.id, product.product.basePrice, 1)
                    }
                  >
                    Add to Cart
                  </Button>
