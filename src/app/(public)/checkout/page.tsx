...
import { GetProductBySlug, getProductBySlug } from '@/actions/product/product';
...
export default function CheckOutScreen() {
  const [cart, setCart] = useState<GetProductBySlug[]>([]);
  ...
  useEffect(() => {
    ...
    fetchAddresses();
    // No need to set selectedAddressId here - we'll select default below
  }, []);
  ...
  async function fetchAddresses() {
    try {
      ...
      // If a default address exists, select it, else the first one, else null
  ...
  const subTotal = cart.reduce((sum, item) => sum + +item?.basePrice! * qty, 0);
  const shippingFee = 150;
  // Calculate discount as the difference between basePrice and salePrice for all items in cart
  // Coupons and offers will be applied later
  const discount = cart.reduce((sum, item) => {
    if (item && item.basePrice && item.salePrice && +item.basePrice > +item.salePrice) {
      return sum + (+item.basePrice - +item.salePrice) * qty;
    }
    return sum;
  }, 0);
  ...
  const handlePlaceOrder = async () => {
    ...
    // Here, send POST to API with order details
    setTimeout(() => {
      setIsPlacingOrder(false);
      router.push('/order-confirmation'); // Next page
    }, 1200);
  };
  ...
  return (
    <div className='max-w-4xl mx-auto p-4 md:py-10 space-y-8'>
                  ...
                  >
                    ...
                    <div>
                      <div className='text-sm text-gray-500'>
                        {formatCurrency(+item?.basePrice!, item?.currency!)} × {qty}
                      </div>
                      <div className='font-semibold ml-4'>
                        {formatCurrency(+item?.basePrice! * qty, item?.currency!)}
                      </div>
                    </div>
                  </li>
                ))}
            ...
            <div className='space-y-1 text-right text-md'>
              <div>
                ...
                <span className='font-semibold'>
                  {formatCurrency(subTotal, cart[0]?.currency || 'PKR')}
                </span>
              </div>
              <div>
                Shipping:{' '}
                <span className='font-semibold'>
                  {formatCurrency(shippingFee, cart[0]?.currency || 'PKR')}
                </span>
              </div>
              <div>
                Discount:{' '}
                <span className='font-semibold text-destructive'>
                  -{formatCurrency(discount, cart[0]?.currency || 'PKR')}
                </span>
              </div>
              <div className='mt-2 text-lg font-bold'>
                Total: <span>{formatCurrency(total, cart[0]?.currency || 'PKR')}</span>
              </div>
            </div>
          </div>
        )}
      ...
      {/* Step 3: Payment method (Placeholder for now) */}
      <section className='bg-white shadow rounded p-4 mb-8'>
      ...
      <div className='text-right'>
        <Button
          size='lg'
          disabled={cart.length === 0 || !selectedAddressId || isPlacingOrder || !agree}
