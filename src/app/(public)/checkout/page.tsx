      ...
      case null:
        getCartItems()
          .then((response) => {
            ...
            // Transform cart items to CheckoutItem
            const items: CheckoutItem[] = response.cartItems.map(
              (item: any) => ({
                ...
                sku: item.sku, // Assuming sku is on cart item or product variant
                variantId: item.variantId,
              })
            );

            const isSameProductIds = items?.every(
              (item) => item.productId === items?.[0].productId
            );
            if (isSameProductIds) {
              setCart([items?.[0]]);
          ...
          .then(({ res }) => {
            ...
            if (res) {
              const item: CheckoutItem = {
                ...
                salePrice: res.salePrice ? Number(res.salePrice) : null,
      ...
      <section className="bg-white flex flex-col shadow rounded p-4 mb-8">
        ...
        {!addressLoading && addresses.length === 0 ? (
          ...
          >
            ...
            />
            <span>
              I confirm that all the address information I have entered is
              accurate and valid. I understand that if the information is
              incorrect, my order may be canceled.
            </span>
          </Label>
        ) : (
          <></>
          ...
          <div>
            ...
            <div className="space-y-1 text-right text-md">
              ...
              <div className="mt-2 text-lg font-bold">
                Total:{" "}
                <span>
                  {formatCurrency(total, safeCart[0]?.currency || "PKR")}
                </span>
              </div>
            </div>
          </div>
