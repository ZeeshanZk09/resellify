...
import { getAllCategoriesFlat, getCategoryBySlugPath } from '@/actions/category/category';
...
export default async function CategoryPage({ params }: CategoryPageProps) {
  ...
  const categoryContext = await getCategoryBySlugPath(decodedCategorySlug, decodedSubcategorySlug);
  ...
  const productsResult = await getCategoryProducts(decodedCategorySlug, decodedSubcategorySlug, 10);
  ...
  const heading = parent && parent.id !== category.id ? `${category.name}` : category.name;
  ...
  return (
    ...
    >
      <div className='max-w-7xl mx-auto'>
        <nav className='text-sm text-slate-500 mt-6 mb-4' aria-label='Breadcrumb'>
        ...
        <header className='mb-6'>
          <h1 className='text-2xl sm:text-3xl font-semibold text-slate-900'>{heading}</h1>
