...
function BillingHistoryContent({ dataPromise }: { dataPromise: Promise<[any, any]> }) {
  const [historyResult, subscriptionResult] = use(dataPromise);
  const [portalLoading, setPortalLoading] = useState(false);

  // Process transactions with type safety
  const transactions: PaymentTransaction[] =
    historyResult.success && historyResult.data
      ? (historyResult.data as any[]).map((t) => ({
          ...t,
          amount: typeof t.amount === 'number' ? t.amount : Number(t.amount),
        }))
      : [];

  // Process subscription with type safety
    ...
    subscriptionResult.success && subscriptionResult.data
      ? ({
          ...
          plan: {
            ...
            price:
              typeof subscriptionResult.data.plan.price === 'number'
                ? subscriptionResult.data.plan.price
                : Number(subscriptionResult.data.plan.price),
          },
        } as Subscription)
      : null;
...
export default function BillingHistory() {
  const [dataPromise] = useState(() =>
    Promise.all([getUserBillingHistory(), getUserSubscription()])
  );

  return (
    <Suspense fallback={<BillingSkeleton />}>
      <BillingHistoryContent dataPromise={dataPromise} />
    </Suspense>
  );
}
