...
const DropDownList = ({ data, width = 'auto', selectedIndex = 0, onChange }: TProps) => {
  ...
  // Fix: Adjust type so useToggleMenu gets RefObject<HTMLDivElement>
  const [isActive, setIsActive] = useToggleMenu(
    false,
    optionRef as React.RefObject<HTMLDivElement>
  );
      ...
      {data.length ? (
        <>
          ...
          >
            <ArrowIcon
              width={8}
              className={cn('fill-none', isActive ? 'stroke-gray-500' : 'stroke-gray-400')}
            />
          </div>
          <div
            className={cn(
